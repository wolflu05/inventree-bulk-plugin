...
from .BulkGenerator.BulkGenerator import (
    BaseFieldDefinition,
    ParseChildReturnElement,
    ParseChildReturnType,
)
...
def get_model_instance(
    model: Model, pk: str, limit_choices={}, error_msg="", allow_multiple=False
):
    ...
    try:
        ...
        raise ValueError(
            f"Model '{model._meta}' where { ({**filters, **limit_choices}) } not found {error_msg}"
        )
...
class FieldDefinition(BaseFieldDefinition):
    name: str
    field_type: Literal[
        "text", "boolean", "number", "float", "model", "select", "list", "object"
    ] = "text"
    ...
    def __post_init__(self):
        if self.cast_func is None and (
            cast_func := self.type_casts.get(self.field_type, None)
        ):
        ...
        if self.generate_type == "tree":
            ...
            if not parent_id:
                if is_create:
                    ...
                    return {
                        "gen": {
                            k: f"<parent '{f.name}'>" for k, f in self.fields.items()
                        }
                    }
...
class StockLocationBulkCreateObject(BulkCreateObject[StockLocation]):
    ...
    fields = {
        ...
        "location_type": FieldDefinition(
            "Location type", field_type="model", model="stock.StockLocationType"
        ),
    }
...
class PartCategoryBulkCreateObject(BulkCreateObject[PartCategory]):
    ...
    fields = {
        "name": FieldDefinition("Name", required=True),
        "description": FieldDefinition("Description"),
        "default_location": FieldDefinition(
            "Default location",
            field_type="model",
            model=("stock.StockLocation", {"structural": False}),
        ),
...
class PartBulkCreateObject(BulkCreateObject[Part]):
    ...
    def get_fields(self):
        return {
            ...
            "category": FieldDefinition(
                ...
                description="If not set, defaults to current category",
            ),
            "variant_of": FieldDefinition(
                "Variant of",
                field_type="model",
                model=("part.Part", {"is_template": True}),
            ),
            ...
            "default_expiry": FieldDefinition(
                "Default Expiry",
                field_type="number",
                description="Expiry time (in days) for stock items of this part",
            ),
            "minimum_stock": FieldDefinition(
                "Minimum Stock",
                field_type="float",
                description="Minimum allowed stock level",
            ),
            "units": FieldDefinition(
                "Units", description="Units of measure for this part"
            ),
            ...
            "responsible_owner": FieldDefinition(
                "Responsible", field_type="model", model="users.owner"
            ),
            # model does not exist, so a custom processor will be used in the frontend
            "image": FieldDefinition(
                "Image",
                field_type="model",
                api_url="/api/part/thumbs/",
                model=("_part.part_image", {}, None),
                description="You can use any already uploaded part picture here, or reference an external URL.",
            ),
            ...
            "attachments": FieldDefinition(
                ...
                items_type=FieldDefinition(
                    ...
                    fields={
                        ...
                        "file_headers": FieldDefinition(
                            "File headers",
                            description="Provide headers that should be used to download the file in json format.",
                        ),
                    },
                    required=True,
                ),
            ...
            "supplier": FieldDefinition(
                ...
                fields={
                    "supplier": FieldDefinition(
                        "Supplier",
                        field_type="model",
                        model=("company.Company", {"is_supplier": True}),
                        required=True,
                    ),
                    ...
                    "_make_default": FieldDefinition(
                        "Make default",
                        field_type="boolean",
                        description="Make this supplier part the default for this part.",
                    ),
                },
            ),
            "manufacturer": FieldDefinition(
                ...
                fields={
                    "manufacturer": FieldDefinition(
                        "Manufacturer",
                        field_type="model",
                        model=("company.Company", {"is_manufacturer": True}),
                        required=True,
                    ),
            ...
            "stock": FieldDefinition(
                ...
                fields={
                    "quantity": FieldDefinition(
                        "Quantity", field_type="float", required=True
                    ),
                    ...
                    "status": FieldDefinition(
                        "Status",
                        field_type="select",
                        get_options=self.get_stock_status_options,
                        default="10",
                    ),
                    "notes": FieldDefinition("Note"),
                    "packaging": FieldDefinition("Packaging"),
                    "purchase_price": FieldDefinition(
                        "Purchase price", field_type="float"
                    ),
                    "purchase_price_currency": FieldDefinition(
                        "Currency",
                        field_type="select",
                        get_options=self.get_currencies_options,
                        get_default=self.get_currency_default,
                    ),
                },
            ),
    ...
    def create_objects(self, objects: ParseChildReturnType) -> list[Part]:
        ...
        for attachment_data in [
            a for p in objects for a in p[0].get("attachments", [])
        ]:
            if attachment_data.get("link", None) and attachment_data.get(
                "file_url", None
            ):
            ...
            if file_url and file_url not in self.attachments:
                try:
                    default_headers = json.loads(
                        registry.get_plugin("inventree-bulk-plugin").get_setting(
                    ...
                    r.raise_for_status()
                    filename = (
                        attachment_data.get("file_name", None)
                        or file_url.split("/")[-1]
                    )
    ...
    def create_object(
        self, data: ParseChildReturnElement, *, parent: Optional[Part] = None
    ):
        ...
        if manufacturer_data:
            ...
            manufacturer = get_model_instance(
                Company, manufacturer_pk, {"is_manufacturer": True}, f"for {part.name}"
            )
        ...
        if supplier_data:
            ...
            supplier = get_model_instance(
                Company, supplier_pk, {"is_supplier": True}, f"for {part.name}"
            )
        ...
        if stock_data:
            ...
            location = get_model_instance(
                StockLocation, location_pk, {}, f"for {part.name}"
            )
        ...
        if related_parts:
            ...
            for related_part in related_parts:
                related_parts_query = get_model_instance(
                    Part, related_part, {}, f"for {part.name}", allow_multiple=True
                ).exclude(pk=part.pk)
    ...
    def get_context(self) -> dict:
        ...
        if (
            variant_of := schema.get("output", {})
            .get("generate", {})
            .get("variant_of", None)
        ):
            template_part = get_model_instance(
                Part, variant_of, {"is_template": True}, "for variant_of field"
            )
            self.parent = (
                template_part  # set parent, used in self.create_objects(...) later
            )
            ctx["gen"] = {
                key: getattr(template_part, key)
                for key in self.fields.keys()
                if hasattr(template_part, key)
            }
        ...
        try:
            ...
            category_dict = {
                key: getattr(self.category, key)
                for key in part_category_fields.keys()
                if hasattr(self.category, key)
            }
    ...
    def get_parameters_default(self):
        ...
        if parent_id:
            try:
                categories = PartCategory.objects.get(pk=parent_id).get_ancestors(
                    include_self=True
                )
                parameters = PartCategoryParameterTemplate.objects.filter(
