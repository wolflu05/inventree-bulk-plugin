...
function EditForm({ template, setTemplate, templateTypeOptions = {}, handleBack }: EditFormProps) {
  ...
  const updateField = useCallback(
    (k: string) => (e: JSX.TargetedEvent<HTMLInputElement, Event>) =>
      setTemplate((t) => ({ ...t, [k]: e.currentTarget.value })),
    [setTemplate],
  );
  const updateTemplate: StateUpdater<BulkDefinitionSchema> = useCallback(
    (valueOrFunc) => {
      setTemplate((t) => ({
        ...t,
        template: typeof valueOrFunc === "function" ? valueOrFunc(t.template) : valueOrFunc,
      }));
    },
    [setTemplate],
  );
  ...
  const generateKeys = useMemo(
    () => allGenerateKeys[template.template_type] || null,
    [allGenerateKeys, template.template_type],
  );
  ...
  const saveOrUpdate = useCallback(() => {
    (async () => {
      ...
      if (200 <= res.status && res.status < 300) {
        ...
        setError(
          `An error occurred. ${Object.entries(data as Record<string, { message: string }[]>)
  ...
  return (
    <div>
      <h5>
        {create ? "Create" : "Edit"} {!create ? `"${template.name}" ` : ""}template
      </h5>
      ...
      <div class="d-flex" style="gap: 5px">
        <button
          class={`btn ${hasChanged && !create ? "btn-outline-secondary" : "btn-outline-primary"}`}
          onClick={handleBack}
          disabled={hasChanged && !create}
        >
          Back
        </button>
        <button
    ...
    return (
      <div>
        <EditForm
          ...
          setTemplate={setEditingTemplate as unknown as StateUpdater<TemplateModel>}
          templateTypeOptions={templateTypeOptions}
          handleBack={backEditing}
        />
      </div>
    );
  }
        ...
        <tbody>
          {savedTemplates.map((template) => (
            <tr>
              ...
              <td>
                <button class="btn btn-sm btn-outline-danger me-1" onClick={startDeleting(template)}>
                  Delete
                </button>
                <button class="btn btn-sm btn-outline-success" onClick={startEditing(template)}>
                  Edit
                </button>
              </td>
            </tr>
