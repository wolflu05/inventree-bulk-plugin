...
export function BulkDefinitionChildSchemaBuilder({ childSchema, setChildSchema, generateKeys, extendsKeys }: BulkDefinitionChildSchemaBuilderProps) {
    ...
    const addDimension = useCallback(() => setChildSchema(s => {
        s.dimensions.push("");
        s.count.push("");
        return { ...s, dimensions: s.dimensions, count: s.count };
    }), [setChildSchema]);
    const removeDimension = useCallback((n: number) => () => setChildSchema(s => ({ ...s, dimensions: s.dimensions.filter((_, i) => i !== n), count: s.count.filter((_, i) => i !== n) })
    ), [setChildSchema]);
    ...
    const addChild = useCallback(() => setChildSchema(s => {
        if (!s.childs) s.childs = [];
        s.childs.push(structuredClone(defaultSchema.output));
        return { ...s, childs: s.childs };
    }), [setChildSchema]);
    const removeChild = useCallback((n: number) => () => setChildSchema(s => {
